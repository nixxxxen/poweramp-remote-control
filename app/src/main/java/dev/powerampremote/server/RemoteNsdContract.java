...
final class RemoteNsdContract {
    ...
    static Map<String, String> wifiDirectAttributes(String serverId, int port) {
        if (port < 1 || port > 65_535) {
            throw new IllegalArgumentException("invalid port");
        }
        Map<String, String> attributes = new LinkedHashMap<>(attributes(serverId));
        attributes.put(ATTRIBUTE_PORT, Integer.toString(port));
        return Collections.unmodifiableMap(attributes);
    }
}
